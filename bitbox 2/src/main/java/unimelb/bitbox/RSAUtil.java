...
public class RSAUtil {
	...
	public static PublicKey GetPubKey (InputStream pub) throws NoSuchAlgorithmException, IOException, InvalidKeySpecException {
		KeyFactory f = KeyFactory.getInstance("RSA");
	    RSAPublicKeySpec pubspec = decodeOpenSSH(readAllBase64Bytes(pub));
	    PublicKey pubKey = f.generatePublic(pubspec);
	    return pubKey;
	}
	
	public static PrivateKey GetPriKey (InputStream pvt) throws NoSuchAlgorithmException, IOException, InvalidKeySpecException {
		KeyFactory f = KeyFactory.getInstance("RSA");
	    RSAPrivateKeySpec prispec = decodeRSAPrivatePKCS1(readAllBase64Bytes(pvt));
	    PrivateKey priKey = f.generatePrivate(prispec);
	    return priKey;
	}
	...
	public static byte[] strToByteArray(String str) throws UnsupportedEncodingException {
	    ...
	    byte[] byteArray = str.getBytes("US_ASCII");
	    return byteArray;
	}
